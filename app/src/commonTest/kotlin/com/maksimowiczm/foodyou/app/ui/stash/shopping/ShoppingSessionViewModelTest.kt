...
class ShoppingSessionViewModelTest {
    @Test
    fun `when products are added one after another, the preview keeps session state and total calories`() = runBlocking {
        val productOne = sampleProduct(id = 1L, name = "Skyr", nutritionFacts = nutritionWithEnergy(60.0))
        val productTwo = sampleProduct(id = 2L, name = "Rice", nutritionFacts = nutritionWithEnergy(130.0))
        ...
        viewModel.updatePendingQuantity("200")
        ...
        assertEquals(1, viewModel.state.value.items.size)
        viewModel.updatePendingQuantity("500")
        viewModel.addPendingProduct()
    ...
    @Test
    fun `when editing an item quantity before confirm, the running calories update`() = runBlocking {
        val product = sampleProduct(id = 1L, name = "Skyr", nutritionFacts = nutritionWithEnergy(60.0))
        ...
        viewModel.saveItemQuantity(itemId)
    ...
    @Test
    fun `when confirming the shopping session, it commits items, emits finish, and clears local state`() = runBlocking {
        val stashRepository = FakeStashRepository(initialStashes = listOf(sampleStash(id = 9L, name = "Pantry")))
        val productOne = sampleProduct(id = 1L, name = "Skyr", nutritionFacts = nutritionWithEnergy(60.0))
        val productTwo = sampleProduct(
            id = 2L,
            name = "Juice",
            isLiquid = true,
        ...
        awaitState(viewModel) { it.items.size == 1 }
        viewModel.selectProduct(searchProduct(productTwo))
        viewModel.updatePendingQuantity("400")
        viewModel.addPendingProduct()
        awaitState(viewModel) { it.items.size == 2 }
    ...
    @Test
    fun `when confirming with a valid edited quantity, the latest edit is committed`() = runBlocking {
        val stashRepository = FakeStashRepository(initialStashes = listOf(sampleStash(id = 9L, name = "Pantry")))
        val product = sampleProduct(id = 1L, name = "Skyr", nutritionFacts = nutritionWithEnergy(60.0))
        val viewModel =
            shoppingSessionViewModel(
    ...
    private fun shoppingSessionViewModel(
        ...
        stashRepository: FakeStashRepository = FakeStashRepository(initialStashes = listOf(sampleStash())),
