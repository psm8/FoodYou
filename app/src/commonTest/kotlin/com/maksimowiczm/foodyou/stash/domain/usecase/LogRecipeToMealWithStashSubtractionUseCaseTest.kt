...
class LogRecipeToMealWithStashSubtractionUseCaseTest {
    ...
    @Test
    fun when_direct_sub_recipe_stash_entry_uses_grams_then_serving_requirement_subtracts_recipe_entry() = runBlocking {
        val flour = sampleProduct(id = 1, name = "Flour", brand = null)
        val dough =
            Recipe(
                id = FoodId.Recipe(9),
        ...
        val movement = stashRepository.allMovements().single()
        assertEquals(1L, movement.itemId.value)
        assertEquals(StashMeasurement.grams(200.0), movement.measurementChange.negate())
        assertEquals("Pizza", diaryRepository.allEntries().single().food.name)
    }

    @Test
    fun when_partial_logging_recipe_with_weight_based_direct_sub_recipe_match_then_product_entries_are_not_subtracted_as_fallback() = runBlocking {
        val flour = sampleProduct(id = 1, name = "Flour", brand = null)
        val dough =
            Recipe(
                id = FoodId.Recipe(9),
        ...
        val movement = stashRepository.allMovements().single()
        assertEquals(1L, movement.itemId.value)
        assertEquals(StashMovementOperation.IngredientSubtract, movement.operation)
        assertEquals(StashMeasurement.grams(100.0), movement.measurementChange.negate())
    }
